...
**## Phase 5: The Predictive Probability & Sentiment Engine (The Master Filter)

**Objective:** To shift from reactive technical analysis to proactive probability modeling. This phase does not predict exact prices, but generates a "Daily Market Bias" (Bullish, Bearish, or Sideways) and calculates the expected trading range to act as the ultimate veto power over Phase 1-4 trades.

### 1. The Pre-Market Data Aggregator
A scheduled script that runs daily at 8:30 AM IST to gather macro-economic context:
* **Global Indices:** Scrapes previous day close of US Markets (S&P500, DJI) and live SGX/GIFT Nifty.
* **Macro Variables:** USD/INR exchange rate, Crude Oil prices, and Bond Yields.
* **Logic:** If Macro data is overwhelmingly negative, automatically reduce capital allocation per trade by 50% for that specific day.

### 2. NLP News Sentiment Analyzer
* **Action:** Fetches the top 50 financial headlines for the day via RSS feeds or news APIs.
* **Processing:** Uses a lightweight Natural Language Processing (NLP) library (like VADER or TextBlob) to assign a sentiment score from -1 (Extremely Bearish) to +1 (Extremely Bullish).
* **Trigger:** If individual stock news has a sentiment score < -0.5, completely BLOCK any 'Buy' signals generated by Phase 1 for that stock.

### 3. F&O Predictive Range Engine
* **Action:** Downloads the latest Option Chain data for Nifty/BankNifty.
* **Calculation:** Identifies the strike prices with the highest Call Open Interest (Resistance) and highest Put Open Interest (Support).
* **Output:** Generates the "Probable Daily Range". The system will not initiate new Trend-Following trades if the current price is within 0.2% of these extreme boundaries.

### 4. The Final Master Logic (The Confluence of All Phases)
Before executing a trade in the Paper Broker (Phase 3), the system checks the Master Veto:
`IF (Phase 1 = PASS) AND (Phase 2 = PATTERN_FOUND) AND (Phase 4 = FAVORABLE_RISK_REWARD)`
`THEN CHECK Phase 5:`
`IF Phase 5 Sentiment == BEARISH AND Trade_Type == BUY -> REJECT TRADE`
`ELSE -> EXECUTE TRADE`**
